...
uint8_t  ucConstantFanRequestNum = 0;
...
/*系统启停常开定频排风风机*/
void vSystem_ExAirFanConstantAlwaysOpen(System* pt)
{
    ...
    //系统常开排风机个数 > 无故障定频排风机，则最大频率开启变频风机 
    if(pThis->ucConstantFanOpenNum > ucConstantNum && pThis->eExAirFanType == TYPE_CONSTANT_VARIABLE && 
    ...
    if(pThis->ucConstantFanOpenNum <= ucConstantNum && pThis->xVariableFanRequest == FALSE && pThis->eExAirFanType == TYPE_CONSTANT_VARIABLE && 
    ...
    if(pThis->ucConstantFanOpenNum < ucRunningNum)  //系统常开排风机个数 < 运行的定频排风机个数  
    {
        for(i=0, n=ucRunningNum - pThis->ucConstantFanOpenNum; n > pThis->ucConstantFanRequestNum-pThis->ucConstantFanOpenNum && i<ucConstantNum; i++)
        {
    ...
    if(pThis->ucConstantFanOpenNum > ucRunningNum)   //系统常开排风机个数 > 运行的定频排风机个数
    {
        for(i=0, n = pThis->ucConstantFanOpenNum-ucRunningNum; n>0 && i<ucConstantNum; i++)
        {
...
void vSystem_ExAirFanConstantCtrl(System* pt)
{
    ...
    if( pThis->eExAirFanType == TYPE_CONSTANT || 
       ...
       (pThis->pExAirFanVariate->xExAirFanErr == TRUE || pThis->pExAirFanVariate->xExAirFanRemote == FALSE)) ) 
    {
        vSystem_ExAirFanConstantAlwaysOpen(pThis);
        
        if(OSTmrStateGet(&pThis->sExAirFanRequestTimeTmr, &err) == OS_TMR_STATE_RUNNING)    
        {
            vSystem_ExAirFanConstantSwitch(pThis);      //处于运行需求定时期间防止风机故障或远程本地状态变化，要随时切换风机
        }
        if(OSTmrStateGet(&pThis->sExAirFanCtrlTmr, &err) != OS_TMR_STATE_RUNNING || ucConstantFanRequestNum != pThis->ucConstantFanRequestNum)
        {
...
void vSystem_ExAirFanBothCtrl(System* pt)
{
    ...
    if(pThis->eExAirFanType != TYPE_CONSTANT_VARIABLE || pExAirFan == NULL || ulExAirFanRated_Vol == 0)
    {
        return;
    }
    vSystem_ExAirFanConstantAlwaysOpen(pThis);
    ...
    else    //否则，每个【排风机控制周期】（默认1800s）周期执行一次以下A、B、C逻辑：
    { 
        if(OSTmrStateGet(&pThis->sExAirFanRequestTimeTmr, &err) == OS_TMR_STATE_RUNNING)    
        {
            vSystem_ExAirFanConstantSwitch(pThis);        //处于运行需求定时期间防止风机故障或远程本地状态变化，要随时切换风机
