...
CLASS(ExAirFan)           /*排风风机*/ 
{
    ...
    uint16_t     usSetFreq;         //设定运行频率 = 实际值*10
    uint16_t     usMinFreq;         //频率下限     = 实际值*10
    uint16_t     usMaxFreq;         //频率上限     = 实际值*10
    ...
    sDigital_IO  sRemote_DI;        //远程/本地DI通道    
                                    
    eFreqType    eFanFreqType;      //风机频率类型
    eCtrlEn      eCtrlCmd;          //启停命令
                                    
    uint8_t      ucTimeCount;       //命令下发时间计数
    
    BOOL         xExAirFanRemote;   //风机远程/本地     
    BOOL         xExAirFanCmd;      //风机命令    
    BOOL         xExAirFanErr;      //风机故障
    BOOL         xExAirFanCtrlErr;  //风机控制故障
    BOOL         xExAirFanCtrl;     //有无风机控制
